...
function isSixSentencePtParagraph(text = "") {
  const sentences = splitIntoSentences(text);
  if (sentences.length !== 6) return false;
  if (!sentences.every((s) => /^Pt\b/.test(s))) return false;
  if (/\b(he|she|they|his|her|their)\b/i.test(text)) return false;
  return true;
}

async function generateReevalClinicalStatement(rawNotes) {
  // Hard fallback (never leave frm_EASI1 empty) — EXACTLY 6 sentences
  const fallback =
  "Pt has been receiving skilled HH PT to address functional mobility deficits secondary to weakness, impaired balance, and gait instability. " +
  "Pt demonstrates limited progress since prior assessment, and current goals are not met with continued deficits in bed mobility, transfers, and gait tolerance. " +
  "Pt continues to require skilled cueing and training for safe functional mobility, fall-risk reduction strategies, and appropriate use of AD as indicated. " +
  "Pt requires ongoing HEP instruction and progression, along with CG education and safety training to improve carryover and reduce caregiver burden. " +
  "Pt POC will continue to emphasize TherEx, TherAct, gait/balance training, and functional training to improve ADL performance and mobility efficiency. " +
  "Pt requires continued skilled HH PT per POC as medically necessary to address remaining impairments, promote safety, and support functional independence.";
  
  const src = (rawNotes || "").toString().trim();
  if (!openai) return fallback;
  
  const prompt = `
You are generating a HOME HEALTH PHYSICAL THERAPY RE-EVALUATION
"Clinical Statement of Assessment Findings and Recommendations" for the OASIS/HH record.
...
- Use "Pt" (not Patient)
- No bullets, no headings, no arrows
- No pronouns (he/she/they/his/her/their)
- Medicare-compliant tone
- If the notes mention goals not met / slow progress / need further PT, explicitly include those concepts.
- Sentence 6 must explicitly state medical necessity for continued skilled HH PT.

SOURCE NOTES (may include an instruction like "Assessment Summary: Give me 6 sentences..."; convert that into the final 6 sentences):
${src || "General re-evaluation findings"}
`.trim();
  ...
  try {
    ...
    if (out && isSixSentencePtParagraph(out)) return out;
    
    // If OpenAI returns wrong format, fall back safely
    return fallback;
  } catch (e) {
    console.log("[PT Re-Eval] OpenAI generation failed → using fallback:", e.message);
    return fallback;
  }
}

// ---------------- PT RE-EVAL SPECIFIC: Clinical Statement ----------------
...
async function fillReevalClinicalStatement(context, aiNotes, aiData = {}) {
  console.log("[PT Re-Eval] Filling Assessment Summary (frm_EASI1) ...");
  
  const frame = await findTemplateScope(context);
  if (!frame) {
    console.log("[PT Re-Eval] Template frame not found for clinical statement.");
    return;
  }
  
  const clinicalBox = await firstVisibleLocator(frame, ["#frm_EASI1", "textarea#frm_EASI1"]);
  if (!clinicalBox) {
    console.log("[PT Re-Eval] frm_EASI1 not found or not visible.");
    return;
  }
  ...
  let existing = "";
  try {
    existing = ((await clinicalBox.inputValue()) || "").trim();
  } catch {
    existing = "";
  }
  
  // Smart overwrite rules:
  // - If meaningful content exists, preserve it.
  // - If empty or boilerplate, overwrite.
  const isGeneric =
  !existing ||
  existing.length < 80 ||
  /goals not met|progressing slowly|need further pt/i.test(existing);
  
  if (existing && !isGeneric) {
    console.log("[PT Re-Eval] frm_EASI1 has meaningful content → leaving unchanged.");
    return;
  }
  
  // Choose best source:
  // 1) If AI extracted a real paragraph (not "give me ..."), use it.
  // 2) Otherwise generate from raw notes (handles "Assessment Summary: Give me 6 sentences..." prompts).
  const extracted = (aiData?.clinicalStatement || "").trim();
  const raw = (aiNotes || "").toString().trim();
  
  let statementText = "";
  if (extracted && !looksLikeInstruction(extracted) && extracted.length > 80) {
    statementText = extracted;
    console.log("[PT Re-Eval] Using extracted clinicalStatement from parser/AI.");
  } else {
    statementText = await generateReevalClinicalStatement(extracted || raw);
    console.log("[PT Re-Eval] Using generated re-eval assessment summary.");
  }
  
  if (!statementText) {
    console.log("[PT Re-Eval] No statement text available; skipping frm_EASI1.");
    return;
  }
  
  try {
    await clinicalBox.fill("");
    await clinicalBox.type(statementText, { delay: 10 });
    console.log("[PT Re-Eval] frm_EASI1 filled (6 sentences).");
  } catch (e) {
    console.log("[PT Re-Eval] Error filling frm_EASI1:", e.message);
  }
}

// ---------------- MAIN BOT ENTRY ----------------

async function runPtReevalBot({
  patientName,
  visitDate,
  taskType,
  timeIn,
  ...
  console.log("🚀 PT Re-Evaluation bot start...");
  
  const { browser, context, page } = await launchBrowserContext();
  
  try {
    const rawNotes = aiNotes || "";
    
    // Detectors so we only overwrite fields when there is info in AI Notes
    const hasDx =
    /medical (dx|diagnosis)\s*:/i.test(rawNotes) || /dx:\s*/i.test(rawNotes);
    
    const hasLiving =
    /(living situation|lives with|lives alone|stairs|steps|home environment|home setup)/i.test(
                                                                                               rawNotes
                                                                                               );
    
    const hasPain = /\bpain\b/i.test(rawNotes);
    const hasEdema = /\bedema\b/i.test(rawNotes);
    const hasNeuro =
    /(oriented|orientation|cognition|cognitive|alert and oriented)/i.test(rawNotes);
    const hasFunc = /(bed mobility|transfer|transfers|gait|ambulat)/i.test(rawNotes);
    
    // 1) Login (only once, using UI creds or env)
    ...
    // 5) Visit basics (ID confirm, time in/out, visit date)
    await fillVisitBasics(context, { timeIn, timeOut, visitDate });
    
    // 6) Parse note → structured data (includes clinicalStatement capture)
    const aiData = await extractNoteDataFromAI(aiNotes, "Re-evaluation");
    
    // 7) Fill vitals (common.js will NOT fill clinical statement for re-eval — by design)
    await fillVitalsAndNarratives(context, aiData);
    
    // 8) Only fill sections if AI notes actually talk about them
    if (hasLiving) await fillLivingSituation(context, aiData);
    else console.log("[PT Re-Eval] No living situation details → skipping; preserving existing.");
    
    if (hasDx) await fillMedDiagnosisAndSubjective(context, aiData);
    else console.log("[PT Re-Eval] No Medical Dx info → skipping.");
    
    if (hasNeuro) await fillNeuroPhysical(context, aiData);
    else console.log("[PT Re-Eval] No neuro/orientation info → skipping.");
    
    if (hasEdema) await fillEdemaSection(context, aiData);
    else console.log("[PT Re-Eval] No edema mentioned → skipping.");
    
    if (hasPain) await fillPainSection(context, aiData);
    else console.log("[PT Re-Eval] No pain mentioned → skipping.");
    
    if (hasFunc) await fillFunctionalSection(context, aiData);
    else console.log("[PT Re-Eval] No functional info → skipping.");
    
    // 9) Re-eval Assessment Summary → frm_EASI1 (ALWAYS attempts; will preserve meaningful existing)
    await fillReevalClinicalStatement(context, rawNotes, aiData);
    
    // 10) Effective Date + Frequency
    await fillFrequencyAndDate(context, aiData, visitDate);
    
    await clickSave(context);
    await wait(2000);
    
    console.log("🎉 PT Re-Evaluation automation complete.");
